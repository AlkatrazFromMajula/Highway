...
namespace UnityEngine.Splines
{
    ...
    public class SplineAnimate : SplineComponent
    {
        ...
        float m_ElapsedTime;
        public event UnityAction OnEndReached;
        public event UnityAction OnLanesChanged;
        ...
        bool IsNullOrEmptyContainer()
        {
            ...
            {
                Debug.LogWarning("SplineAnimate does not have a valid SplineContainer set.");
                return true;
            }
        ...
        ///// <summary>
        ///// Predicts normalized time overflow 
        ///// </summary>
        ///// <param name="newElapsedTime"></param>
        ///// <returns> true if normalised time exceeds 1, otherwise returns false </returns>
        //public bool PredictNormalizedTimeOverflow(float newElapsedTime)
        //{
        //    var t = newElapsedTime / m_Duration;
        //    float predicted_NormalizedTime = Mathf.Floor(m_NormalizedTime) + t;

        //    return predicted_NormalizedTime >= 1f;
        //}
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerator LaneChangeCooldown()
        {
            Spline spline = new Spline();
            for (int i = 0; i < 2; i++) { spline.Add(Container.Spline.Knots.ToArray()[i]); }

            Vector3 prevFramePos = transform.position;
            float distance = 0;

            while (distance < spline.GetLength()) 
            { 
                yield return new WaitForEndOfFrame();
                distance += (prevFramePos - transform.position).magnitude;
                prevFramePos = transform.position;
            }

            if (OnLanesChanged != null) 
                OnLanesChanged.Invoke();
        }
